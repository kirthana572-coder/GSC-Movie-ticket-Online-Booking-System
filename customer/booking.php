...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { header("Location: " . BASE_URL . "/customer/movies.php"); exit(); }
...
if (!empty($ticket_types_str)) {
    foreach (explode(',', $ticket_types_str) as $pair) { $parts = explode(':', $pair); if (count($parts)===2) $ticketMap[$parts[0]] = $parts[1]; }
}
if (empty($showtime_id) || empty($seat_ids)) { $_SESSION['error'] = "Please select at least one seat."; header("Location: " . BASE_URL . "/customer/select_seat.php?showtime_id=" . intval($showtime_id)); exit(); }
...
try {
    foreach ($seat_ids as $sid) {
        ...
        if (!$seat || $seat['status'] !== 'available') throw new Exception("Seat $sid is no longer available.");
    ...
    foreach ($seat_ids as $sid) { $ticketType = $ticketMap[$sid] ?? 'Adult'; $stmt->bind_param("iis", $booking_id, $sid, $ticketType); $stmt->execute(); }
    $stmt->close();
    ...
    exit();
} catch (Exception $e) { $conn->rollback(); $_SESSION['error'] = "Booking failed: " . $e->getMessage(); header("Location: " . BASE_URL . "/customer/select_seat.php?showtime_id=" . intval($showtime_id)); exit(); }
?>