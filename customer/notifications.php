...
if (isset($_GET['mark_all'])) { 
    $conn->query("UPDATE notifications SET is_read = 1 WHERE user_id = {$_SESSION['user_id']}"); header("Location: " . BASE_URL . "/customer/notifications.php"); 
    exit(); 
}
    ...
    <div class="notification-card">
        ...
        <?php if ($notifications->num_rows > 0): ?>
            <?php while($n = $notifications->fetch_assoc()): ?>
                <a href="?mark_read=<?= $n['id'] ?>" class="notification-item <?= !$n['is_read'] ? 'notification-unread' : '' ?>">
                    <div class="notification-message"><?= htmlspecialchars($n['message']) ?></div> 
                    <div class="notification-time"><?= $n['created_at'] ?></div></a>
            <?php endwhile; ?>
        <?php else: ?>
