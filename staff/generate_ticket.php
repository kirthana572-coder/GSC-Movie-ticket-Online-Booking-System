...
$booking = $conn->query("SELECT b.id, b.payment_status, b.qr_used, b.booking_date, u.full_name, m.title, s.show_date, s.show_time, br.name AS branch_name, GROUP_CONCAT(CONCAT(se.seat_number,' (',bs.ticket_type,')') SEPARATOR ', ') AS seats FROM bookings b JOIN users u ON b.user_id = u.id JOIN showtimes s ON b.showtime_id = s.id JOIN movies m ON s.movie_id = m.id JOIN branches br ON s.branch_id = br.id LEFT JOIN booking_seats bs ON b.id = bs.booking_id LEFT JOIN seats se ON bs.seat_id = se.id WHERE b.id = " . intval($booking_id) . " AND b.payment_status = 'Paid' GROUP BY b.id")->fetch_assoc();
...
<div class="info-row"><span class="label">Customer</span><span class="value"><?= htmlspecialchars($booking['full_name']) ?></span></div>
